...
pacman -Syu --noconfirm base-devel texlive neovim zip unzip zstd gzip pigz ripgrep exa dust fish git rustup nodejs npm starship pandoc wasm-pack go dotnet-sdk aspnet-runtime clang docker mold lazygit bacon xdg-utils tokei openssh gnuplot bat pandoc-plot mdbook fd jq tree-sitter-cli jdk17-openjdk gradle sudo 
